...
def sprint(message, type:bool):
    if type == True:
        print(f"{timestamp()} {Style.BRIGHT}{Fore.YELLOW}INFO {Fore.BLACK}> {Fore.WHITE}{message}{Fore.RESET}{Style.RESET_ALL}")
    if type == False:
        print(f"{timestamp()} {Style.BRIGHT}{Fore.RED}ERROR {Fore.BLACK}> {Fore.WHITE}{message}{Fore.RESET}{Style.RESET_ALL}")
...
def thread_boost(invite, amount, months, nick, bio):
    ...
    while variables.boosts_done != amount:
        ...
        if len(tokens) == 0 or len(tokens) < numTokens:
            sprint(f"Not enough {months} month tokens", False)
            return False
